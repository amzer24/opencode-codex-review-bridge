...
function Install-Staged {
    ...
    Write-Host "$Label $Version to $InstallDir"

    $stage = [System.IO.Path]::GetTempPath() + [System.IO.Path]::GetRandomFileName()
    New-Item -ItemType Directory -Path $stage | Out-Null

    try {
        # Clone at pinned tag into staging area
        $gitResult = git -c advice.detachedHead=false clone --quiet `
            --branch $Version --depth 1 $Repo $stage 2>&1
